...
if __name__ == '__main__':
    ...
    transformations['transformations'] = transformations['transformations'].apply(
        lambda transformation: tuple(itertools.chain.from_iterable(transformation))
    )
    # Generate representation.
    transformations['substitutions'] = transformations['transformations'].apply(
        lambda instances: ''.join(instance.substitution for instance in instances)
    )
    # Generate frequency of each sub-representation.
    transformations['frequencies'] = transformations['transformations'].apply(
        lambda instances: ''.join(f'{instance.substitution}{{{instance.frequency}}}' if instance.frequency > 1 else instance.substitution for instance in instances)
    )
